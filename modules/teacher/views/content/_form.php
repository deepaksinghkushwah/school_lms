<h1><?=$model->isNewRecord ? 'Create Content' : 'Update Content'?></h1><hr>
<div class="page-form">
    <div class="nav-tabs-custom">
        <ul class="nav nav-tabs">
            ...
            <li class="<?= Yii::$app->request->get('section') == 'attachment' ? 'active' : ''; ?> <?=$model->isNewRecord ? 'diabled disabledTab' : ''?>">
                <a title="<?=$model->isNewRecord ? 'You must first create new content, then attach files' : 'Add Attachments'?>" data-toggle="tab" href="#tab_2">Attachments</a>
            </li>     
            <?php } ?>
        ...
        <div class="tab-content">
            ...
            <?php if (!$model->isNewRecord) { ?>
                <div id="tab_2" class="tab-pane <?= Yii::$app->request->get('section') == 'attachment' ? 'active' : ''; ?>">
                    <?= $this->render('_form_attachment', ['model' => $model]) ?>
                </div><!-- /.tab-pane -->
            <?php } ?>
        </div><!-- /.tab-content -->
    </div>
